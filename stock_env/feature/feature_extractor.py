...
class TrendFeatures(BaseFeaturesExtractor):
    ...
    def _modify_entry_exit(self, df: pd.DataFrame):
        ...
        try:
            df["trends_"] = df["trends_"].interpolate(
                limit_area="inside", method="nearest"
            )
        except ValueError:
            df["trends_"] = df["trends_"].fillna(0)
    ...
    def _breakout_entry(self, df: pd.DataFrame):
        ...
        pattern = (
            df.ta.cdl_pattern(
                name=["closingmarubozu", "marubozu", "engulfing", "longline"], scalar=1
            )
            .sum(axis=1)
            .astype(int)
        )
        ...
        df["entry"] = (
            ((pattern > 0) * volume_breakout * (df["close"] > df["HIGH_10"].shift()))
            .astype(bool)
            .astype(int)
        )
        ...
        df["trends"] = (
            df.groupby((df.trends != df.trends.shift()).cumsum())
            .apply(self._modify_entry_exit)
            .to_list()
        )
        return df
